...
{
    cout << "Выберите количество фигур определенных типов, которые хотите рассатвить" << endl;
    cout << "Ввводится сначала за белых, потом за черных, за каждую сторону всегда будет как минимум король, конь, слон" << endl;
    ...
    while (!data_correct) {
        ...
        for (int i = 0; i < 8; ++i) {
            if ((write_figure[name_figure[i]] < 0) || (write_figure[name_figure[i]] > figure_limits[name_figure[i][0]])) {
                data_correct = false;
                cout << "вы что-то неправильно ввели" << endl;
                break;
            }
        }
    }
...
void search_sucessfull_way() 
{
    ...
    playing_board.print();
    cout << "начинается перебор вариантов..." << endl;
    ...
    cout << "пожалуйста, подождите, скоро все выведется" << endl;
    const int write_q = 3;
